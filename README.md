...
- 底部事件达到 `FOCUS` 后，由独立 Python 进程按需采集此前 120 个交易日日 K，WebAPI 使用 `pair-wave-bottom-v3` 在内存中计算 0～100 分波段信号（候选 70～84，强确认 85～100）并写回事件；评分详情保留逐项实际得分和指标证据，历史事件按不可变的 `focused_at` 时间点重算，不引入后续行情；
